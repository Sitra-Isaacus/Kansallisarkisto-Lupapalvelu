 ...
 *
    ...
    php tests/test_hae_saapuneet_lausuntopyynnot.php
 *
...
try{
	...
	$respObj = $logic->hae_saapuneet_lausuntopyynnot($syoteparametrit);
	if ($format == 'json') jsonOut($respObj);

	$res = std2ArrayRecursive($respObj);
	
	print_r($res[1]["LausuntopyynnotDTO"][0]["LausuntoDTO"]["ID"]);
